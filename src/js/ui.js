  ...
  return {
    init (stateChangedListener) {
      ...
      startYear.addEventListener('change', e => {
        ...
        const newEndValue = endYear.value < value ? value : endYear.value;
        initDropDown(endYear, value, 2006, newEndValue);
        updateState();
      }, false);

      endYear.addEventListener('change', e => {
        const value = e.currentTarget.value;
        const newStartValue = startYear.value > value ? value : startYear.value;
        initDropDown(startYear, 1881, e.currentTarget.value, newStartValue);
        updateState();
      }, false);

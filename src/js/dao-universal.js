(function (global, exportedObject) {
  ...
  getObjectStore (db, storageName, mode) {
    return db.transaction(storageName, mode).objectStore(storageName);
  }

  hasData (storageName) {
    return new Promise(resolve => {
      this.dbPromise.then(db => {
        const store = this.getObjectStore(db, storageName, 'readonly');
  ...
  fillDb (storageName, map) {
    return new Promise(resolve => {
      this.dbPromise.then(db => {
        const store = this.getObjectStore(db, storageName, 'readwrite');
